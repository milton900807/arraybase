...
import com.arraybase.tm.ResultMetaData;
...
public class SQLToSolr {
    ...
    private GBLogger log = GBLogger.getLogger(SQLToSolr.class);
    private DBConnectionManager dbcm = new DBConnectionManager();
    //private String exec_job_id = null;
...
//               abc.delete();
        ...
        for (int i = 0; i < thread_count; i++) {
            new JeffsIndexer(_user, "", _query_config, _schema_config,
    ...
    class JeffsIndexer extends Thread {
        ...
        private GBLogger log = GBLogger.getLogger(JeffsIndexer.class);

        public JeffsIndexer(String _user,
        ...
        public String runIndex() throws DBProcessFailedException {
            ...
            try {
                ...
                // load any jar that is in the plugins section
                String plugins = query_config.get(ABQFile.PLUGINS);
                if ( plugins != null )
                {
                    // as of april 2019 this is not fully implemented.
                    AB.loadJars ( plugins );
                }
                ...
                while (total_count < MAX_COUNT && hasmore) {
                    ...
                    if (total_count == 0) {
                        ...
                        for (int i = 0; i < column_count; i++) {
                            ...
                            if (exportFields != null) {
                                if (inlist(exportFields, field_name)) {
                                    solr_config.put(field_name.trim().toLowerCase(),
                                            field_properties);
                                    fields.put(field_name.trim().toLowerCase(), type);
                                }
                            } else {
                                solr_config.put(field_name.trim().toLowerCase(), field_properties);
                                fields.put(field_name.trim().toLowerCase(), type);
                            }
                        }
                        String field_list = "";
                        for (String f : solr_config.keySet()) {
                            field_list += f + "\t";
                        }
                        ...
                        for (String fun : function_fields) {
                            ...
                            solr_config.put(fun, function_field_properties);
                        }
...
//                    GBLogger.status("\t\tQtime\t " + times + "\n");
                    while (res.next()) {
                        ...
                        for (String fun : functionFields) {
                            ...
                            if (abqfunction != null) {
                                ...
                                Object evaluate = functionFeature.evaluateFunction(fun, functioncode, new FunctionFeatureValues(getSolrValues(sid)));
                                if (evaluate != null) {
        ...
        private Map<String, Object> getSolrValues(SolrInputDocument sid) {
            ...
            while ( fi.hasNext() )
            {
                ...
                SolrInputField si = sid.get(field);
                Object object_value = si.getValue();
                v.put ( field, object_value);
            }
            return v;
        }
